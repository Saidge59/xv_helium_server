...
data.sign = {
    ['HS256'] = function(data, key)
        return HMAC.SHA256(key, data, false)
    end,
    ['HS384'] = function(data, key)
        return HMAC.SHA384(key, data, false)
    end,
    ['HS512'] = function(data, key)
        return HMAC.SHA512(key, data, false)
    end,
...
data.verify = {
    ['HS256'] = function(data, signature, key)
        return signature == HMAC.SHA256(key, data, false)
    end,
    ['HS384'] = function(data, signature, key)
        return signature == HMAC.SHA384(key, data, false)
    end,
    ['HS512'] = function(data, signature, key)
        return signature == HMAC.SHA512(key, data, false)
    end,
...
function data:decode(header, str, options)
    ...
    if dotSecond then
        ...
        if options and options.keys and options.keys.public then
            return nil, "Invalid token: no signature"
        end
        bodyStr = str
    end
    ...
    if signature then
        if not self.verify[header.alg](rawHeader .. "." .. bodyStr, signature, options.keys.public) then
            return nil, "Invalid token: signature verify failed"
        end
    end
